<template>
  <view class="page bg-[#0A0E17] pt-12 pb-8 px-6 min-h-screen text-white font-body flex flex-col relative">
    ...
    <!-- 待确认提示 -->
    <view class="bg-red-500/10 border border-red-500/50 rounded-xl p-4 mb-8">
      <view class="flex items-center gap-3">
        <text class="text-red-500 text-xl">⚠</text>
        <view>
          <text class="text-sm font-bold text-red-400">等待您的确认</text>
    ...
    <!-- 比分展示 -->
    <view class="flex justify-center items-center gap-6 mb-10">
      <view class="w-24 h-28 bg-card-bg rounded-2xl flex items-center justify-center border-b-2 border-tennis-neon">
        <text class="font-display text-6xl font-bold text-white">{{ submittedScoreA }}</text>
      </view>

      <text class="font-display font-bold text-3xl text-gray-600">:</text>

      <view class="w-24 h-28 bg-card-bg rounded-2xl flex items-center justify-center border-b-2 border-blue-500">
        <text class="font-display text-6xl font-bold text-white">{{ submittedScoreB }}</text>
      </view>
    </view>
    ...
    <view class="text-center mb-10">
      <view
        ...
        :class="winner === 'A' ? 'bg-tennis-neon/20 text-tennis-neon' : 'bg-blue-500/20 text-blue-400'"
      >
        <text class="text-lg">{{ winner === 'A' ? '🏆' : '🏆' }}</text>
        <text class="font-bold">{{ winner === 'A' ? teamA.name : teamB.name }}</text>
        <text class="font-bold">获胜</text>
      </view>
    </view>

    <!-- 驳回原因（如果有） -->
    <view v-if="rejectReason" class="bg-orange-500/10 border border-orange-500/50 rounded-xl p-4 mb-8">
      <view class="flex items-start gap-3">
        <text class="text-orange-500 text-xl mt-0.5">⚠</text>
        <view>
          <text class="text-sm font-bold text-orange-400">上轮被驳回</text>
    ...
    <view class="text-center mb-8">
      <text class="text-xs text-gray-500">
        ...
        <text
          v-for="n in 3"
          :key="n"
          class="inline-block w-5 h-5 rounded-full text-xs font-bold flex items-center justify-center mx-0.5"
    ...
    <!-- 操作按钮 -->
    <view class="mt-auto flex flex-col gap-3 pb-[env(safe-area-inset-bottom)]">
      <!-- 确认按钮 -->
      <view
        v-if="!isProcessing"
        @tap="confirmScore"
        class="w-full bg-tennis-neon text-dark-slate font-bold py-4 rounded-xl text-lg flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
      ...
      <view
        v-if="!isProcessing"
        @tap="showRejectModal"
        class="w-full bg-red-500/10 border border-red-500/50 text-red-400 font-bold py-4 rounded-xl text-lg flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
        <text class="text-xl">✕</text>
        <text>比分有误，驳回</text>
      </view>

      <text class="text-center text-[10px] text-gray-500 mt-2">
        请仔细核对线下实际比赛结果后再确认
      </text>
    </view>
    ...
    <view v-if="rejectModalVisible" class="fixed inset-0 bg-black/60 z-50 flex items-end justify-center">
      <view class="bg-card-bg w-full rounded-t-3xl p-6 pb-10">
        ...
        <view
          v-if="!isRejecting"
          @tap="confirmReject"
          class="w-full bg-red-500 text-white font-bold py-4 rounded-xl text-lg flex items-center justify-center active:scale-95 transition-transform"
        >
          <text>确认驳回</text>
        </view>
        <view
...
const startCountdown = () => {
  // 模拟12小时倒计时
  const totalSeconds = 12 * 60 * 60 - 1000; // 模拟已经过了1000秒
...
const confirmScore = async () => {
  ...
  try {
    // 实际应调用API
    // const res = await request('POST', '/api/match/confirm', {
...
const confirmReject = async () => {
  ...
  if (selectedRejectReason.value === '比分反了（实际是X:Y）') {
    uni.showToast({ title: '请选择具体原因', icon: 'none' });
    return;
  }
  ...
  try {
    // 实际应调用API
    // const res = await request('POST', '/api/match/reject', {
    //   matchId: matchId.value,
    //   userId: currentUserId,
