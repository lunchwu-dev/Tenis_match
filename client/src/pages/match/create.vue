<template>
  <view class="page bg-dark-slate text-white font-body relative room-root">
    <!-- 顶部导航 -->
    <view
      class="header px-6"
    ...
    >
      <!-- 模式切换：单打/双打 -->
      <view class="bg-card-bg p-1 rounded-xl flex mb-6">
        <view
          @tap="switchMode(1)"
        ...
        <view class="bg-card-bg border border-tennis-neon/30 rounded-2xl overflow-hidden">
          ...
          >
            <view class="flex items-center gap-3">
              ...
              ></image>
              <view>
                <text v-if="idx === 0" class="text-[10px] text-tennis-neon font-bold uppercase tracking-wider block mb-1">{{ isCreator ? '房主' : '队员' }}</text>
      ...
      <!-- 平均分显示 -->
      <view class="flex justify-center gap-8 mb-4">
    ...
    >
      ...
      <text class="text-center text-[10px] text-gray-500 mt-3 block">
...
// footer ≈ invite(64+12) + start(64) + tip(24) + padding ≈ 190，保守留 210
const scrollStyle = computed(() => ({
  height: `calc(100vh - ${statusBarHeight.value + 84}px - 210px - env(safe-area-inset-bottom))`,
  width: '100%',
}));
